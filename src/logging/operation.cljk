(ns logging.operation
  "LoggingOperationActor -- one logging operation = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor (LoggingAdvisor)
  is sealed into a single node (:advise); its proposal is ALWAYS routed
  through the Logging Coordination Governor (:govern) and the rollout phase gate
...
(defn build
  "Compiles a LoggingOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `logging.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; LoggingAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Logging Coordination Governor -- independent censor.
      (g/add-node :govern
